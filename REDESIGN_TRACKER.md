...
- Pass 3: Mobile lead capture header. Scope: keep a persistent Contact Us CTA visible in the mobile sticky header while preserving the hamburger navigation. Status: deployed live in Render deploy `dep-d7l7rkjrjlhs73edgaig` from commit `600277f`; live 320px header verification passed.
- Pass 4: Premium industry imagery. Scope: replace low-resolution mixed industry photos with a cohesive generated 1600x1200 editorial set while preserving the old source assets. Status: deployed live in Render deploy `dep-d7l8dnosfn5c73f40k80` from commit `e9f1671`; live asset resolution and homepage image-load checks passed.
